...
def draw_simulation(screen, double_pendulum, x_offset, y_offset, scale=100):
    x, x_dot, theta1, theta1_dot, theta2, theta2_dot = double_pendulum.state
    ...
    # Render First Pole
    # Calculate first pole position
    pole1_len = double_pendulum.length1 * 2 * scale
    pole1_end_x = cart_x + math.sin(theta1) * pole1_len
    pole1_end_y = cart_y - math.cos(theta1) * pole1_len
    
    pygame.draw.line(screen, POLE_COLOR, (cart_x, cart_y), (pole1_end_x, pole1_end_y), 6)
    
    # Render Second Pole
    # Calculate second pole position (based on absolute angles)
    
    pole2_len = double_pendulum.length2 * 2 * scale
    pole2_end_x = pole1_end_x + math.sin(theta2) * pole2_len
    pole2_end_y = pole1_end_y - math.cos(theta2) * pole2_len
    
    pygame.draw.line(screen, POLE2_COLOR, (pole1_end_x, pole1_end_y), (pole2_end_x, pole2_end_y), 6)
    
    # Render Joints
    pygame.draw.circle(screen, RED, (int(cart_x), int(cart_y)), 5)
    pygame.draw.circle(screen, RED, (int(pole1_end_x), int(pole1_end_y)), 5)
...
def draw_network(screen, genome, x, y, width, height):
    ...
    for node in genome.nodes:
        ...
        if node.type == 'INPUT': 
            ...
            elif node.id == 2: label = "t1"
            elif node.id == 3: label = "dt1"
            elif node.id == 4: label = "t2"
            elif node.id == 5: label = "dt2"
        elif node.type == 'OUTPUT': 
            color = (255, 255, 0)
            if node.id == 6: label = "F"
        ...
        if dist < 10:
            ...
            if node.type == 'INPUT': 
                ...
                elif node.id == 2: desc = "Pole 1 Angle"
                elif node.id == 3: desc = "Pole 1 Angular Velocity"
                elif node.id == 4: desc = "Pole 2 Angle"
                elif node.id == 5: desc = "Pole 2 Angular Velocity"
            elif node.type == 'OUTPUT': 
                if node.id == 6: desc = "Force Application"
