...
class Genome:
    ...
    def mutate(self, innovation_history):
        ...
        # Apply node addition mutation with 25% probability
        if random.random() < 0.25:
            self.mutate_add_node(innovation_history)

        # Apply link addition mutation with 10% probability
    ...
    def mutate_add_node(self, innovation_history):
        ...
        # Determine the layer of the new node (average of input and output node layers)
    ...
    def __init__(self, size=POPULATION_SIZE):
        ...
        for _ in range(size):
            ...
            for i in range(OUTPUT_NODES):
                g.add_node(NodeGene(INPUT_NODES + i, 'OUTPUT', 1.0))
            
            # Create initial random connections
            for i in range(INPUT_NODES):
                for j in range(OUTPUT_NODES):
                     innov = self.innovation_history.get_innovation(i, INPUT_NODES + j)
                     g.add_connection(ConnectionGene(i, INPUT_NODES + j, random.uniform(-1, 1), True, innov))
